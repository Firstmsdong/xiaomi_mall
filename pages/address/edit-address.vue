	...
	<view class="">
		...
		<view class="u-f setDefault u-f-jsb">
			...
			<view class="setDefault-switch">
				<template v-if="defaultAdres == 1">
					<switch checked="true" @change="switch1Change" style="transform:scale(0.7)" color="#0BB8EB"/>
				</template>
				<template v-if="defaultAdres == 0">
					<switch  @change="switch1Change" style="transform:scale(0.7)" color="#0BB8EB"/>
		...
		<!-- 保存收货地址 -->
		<view class="conserveAds u-f-ajc" @click="conserverAddress">修改收货地址</view>
		...
		methods: {
			// 三级联动相关逻辑
			 ...
			 getData(){
				 ...
				 this.$request('/member/getAddressInfo',{id:this.addressId})
				 .then(res=>{
					 console.log(res)
				 	this.name=res.data.username
					this.sexIndex = res.data.sex
					this.phone = res.data.telphone
					this.text = res.data.city
					this.addressDetil = res.data.address
					this.defaultAdres = res.data.default
